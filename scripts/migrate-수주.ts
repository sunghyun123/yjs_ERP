 ...
 * [보관용 아카이브 — 실행하지 않음]
 *
 * 이 파일은 구ERP 엑셀 → Supabase 초기 데이터 이관에 실제로 쓰인 스크립트다.
 * 이관은 2026-06에 끝났고, 원본 엑셀은 레포 밖으로 옮겼기 때문에 지금은 실행되지 않는다.
 * (package.json 의 migrate:all 스크립트도 함께 제거)
 *
 * 코드를 지우지 않고 통째로 주석처리해 남겨 둔 이유:
 *   "데이터를 어떻게 옮겼나"(파싱 규칙·중복 처리·배치 크기·검증)를
 *   나중에 설명해야 할 때, 요약문이 아니라 실제로 돌았던 코드가 근거가 되기 때문.
 *
 * 아래 원본 코드 전문. 되살리려면 각 줄의 '// ' 접두사를 제거하고
 * 루트에 원본 엑셀(수주대장조회.xlsx)을 다시 놓으면 된다.
 */
