...
> 최종 업데이트: 2026-06-24 (변경내역 추적 페이지 추가 — audit_log 트리거 기반)
...
| 62 | 공사현장 드롭다운 + 준공 자동화 (실배포 후 첫 운영 피드백) — **①공사현장 드롭다운화**: 공사 구분·종류는 드롭다운인데 공사현장만 자유입력이라 통일성↓(유저 피드백). 새 `공사현장` 테이블(RLS: 읽기 화이트리스트·쓰기 admin) + `/admin/sites` 추가/삭제 + OrderForm Select 교체. `수주.공사현장`은 문자열 유지(A-1, 비정규화)라 현장 삭제해도 기존 수주 안 깨짐. **②준공 자동화**: 준공을 찍어도 달성률이 안 바뀌고(저장 컬럼 `수주.달성율`을 갱신하는 코드가 없었음), 준공액≠수주액인데 매출손익은 수주액 기준이라 정확도↓. 준공 저장 시 `수주.달성율=100` + `공사이력`에 `준공정산=true` 행 한 줄 upsert(성과=준공액공급가−기존누계) → 매출손익 정본(공사이력 합산) 안 깨고 총성과=준공액. **③무손실 해제**: 실수 준공 대비, 해제는 `eq('준공정산',true)`로 자동 행만 삭제(직접 입력 공사이력 보존)+달성율 재계산. **④두 달성률 구분**: 공사이력 기반 공정 달성률(추정치, 100% 초과 허용·clamp 금지)과 수주대장 준공 플래그(=100)를 분리, 준공은 수동 전용(공사이력 100%가 자동 준공 트리거 안 됨). ⑤준공 공사는 `/progress` 입력 드롭다운에서 제외. 계산은 순수 함수(`completion.ts`)로 분리+단위테스트. 피드백 기록=`docs/feedback/운영-피드백-기록.md` | ✅ |
| 64 | 변경내역 추적 (`/admin/updates`, 파일럿 배포 직후 운영 보조) — 신규 ERP가 안정화될 때까지 레거시 ERP에 변경을 이중입력해야 해서, "최근 무엇이 들어왔는지"를 빠짐없이 조회. **DB 트리거 방식 채택**(앱레벨 로깅 기각: 10+ mutation 자리 손봐야·하나라도 빠지면 누락, 특히 행이 사라지는 삭제). 9개 테이블(수주·기성·공사이력·투입실적·투입실적상세 + 마스터 4종)에 `AFTER INSERT/UPDATE/DELETE` 범용 트리거 → `audit_log`(table_name·operation·row_id·old_data/new_data jsonb·changed_at) 적재. **함정 3종**: ①`audit_log`에 RLS 켜면 일반 mutation의 트리거 INSERT가 막혀 *원래 작업까지* 실패 → `security definer`로 소유자 권한 우회 ②SELECT(admin) 정책만 두고 INSERT 정책 없음 → 클라 직접 조작 불가, 기록은 트리거만 ③`공사이력`엔 생성일 컬럼이 없어 소급입력(작업일자=지난주, 입력=오늘)이 날짜 조회로 안 잡힘 → `changed_at`(실제 insert 시각)이 메움. 화면=서버(기간 URL searchParams 조회 + FK id→이름 맵)/클라(분류칩 필터·행 펼침 전체 스냅샷). 분류 라벨·요약은 순수함수(`category.ts`·`format.ts`, 투입실적상세는 투입실적 그룹으로 묶음·라벨은 그룹명 기준)+단위테스트 15개. **DB 적용·트리거 검증 완료**(거래처 INSERT/UPDATE/DELETE 3행 정확 포착, 삭제는 old_data만·등록은 new_data만). 한시적 도구(신규 ERP 단독운영 정착 시 트리거·테이블 드롭). 마이그=`supabase/migrations/20260624120000_audit_log.sql` | ✅ |
| 63 | 공사이력 성과 원/% 토글 입력 (운영 피드백, 김도윤 차장) — 규모 큰 공사는 원단위 입력이 무의미(20억짜리 100만원어치=1%↓)하고, 소액 공사는 금액 계산이 더 번거로움 → **성과 입력에 `[원\|%]` 토글(기본 %)** 추가. % 입력은 `이번 증분 Δ%` 의미. **단독 소스 불변**: 어떤 방식으로 입력하든 DB에는 항상 `성과금액`(원)만 저장(% 컬럼 미추가, `handleSave`·달성률 계산 무변경) → 매출손익 합산 안 깨짐. 환산식 `원=round(%/100×하도적용금액)`은 순수 헬퍼(`progress/_lib/percent.ts`)로 분리+단위테스트 10개. 정본(원)과 표시(%) 문자열 분리로 타이핑 떨림 방지, 하도적용금액 없으면 % 비활성·원 모드 강제, 100% 초과는 가드/경고 없이 허용. Δ카드에 '이번 증분 금액'을 1급 값으로 부각(주간 보고서 복붙 편의). 피드백 기록=`docs/feedback/운영-피드백-기록.md` | ✅ |
